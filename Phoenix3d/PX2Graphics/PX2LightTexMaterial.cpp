...
PX2_IMPLEMENT_RTTI(PX2, Material, LightTexMaterial);
PX2_IMPLEMENT_STREAM(LightTexMaterial);
PX2_IMPLEMENT_FACTORY(LightTexMaterial);
PX2_IMPLEMENT_DEFAULT_NAMES(Material, LightTexMaterial);

//----------------------------------------------------------------------------
LightTexMaterial::LightTexMaterial ()
{
	VertexShader* vshader = new0 VertexShader("PX2.LightTex",
		3, 4, 6, 0, false);
	...
	vshader->SetConstant(3, "LightWorldDirection", 1);
	vshader->SetConstant(4, "LightSpecular", 1);
	vshader->SetConstant(5, "FogParam", 1);
	vshader->SetBaseRegisters(msVRegisters);
	vshader->SetPrograms(msVPrograms);

	PixelShader* pshader = new0 PixelShader("PX2.LightTex",
		3, 1, 1, 2, false);
	...
	vshader->SetShaderKey(SKT_LIGHTTEX);
	pshader->SetShaderKey(SKT_LIGHTTEX);
}
//----------------------------------------------------------------------------
LightTexMaterial::~LightTexMaterial ()
{
...
//----------------------------------------------------------------------------
PixelShader* LightTexMaterial::GetPixelShader () const
{
	return mTechniques[0]->GetPass(0)->GetPixelShader();
}
//----------------------------------------------------------------------------
MaterialInstance* LightTexMaterial::CreateInstance (Texture2D* texture0,
	Texture2D* texture1, Light *dirLight) const
{
	...
	instance->SetVertexConstant(0, 3, new0 LightWorldDVectorConstant(dirLight));
	instance->SetVertexConstant(0, 4, new0 LightSpecularConstant(dirLight));
	instance->SetVertexConstant(0, 5, new0 FogParamConstant());
...
//----------------------------------------------------------------------------
void LightTexMaterial::_CalShaderKey ()
{
	...
	{
		vshader->SetShaderKey(SKT_LIGHTTEX_ALPHATEST);
		pshader->SetShaderKey(SKT_LIGHTTEX_ALPHATEST);
		pshader->SetPrograms(msPPrograms);
	}
	else
	{
...
// Streaming support.
//----------------------------------------------------------------------------
LightTexMaterial::LightTexMaterial (LoadConstructor value)
	:
Material(value)
{
}
//----------------------------------------------------------------------------
void LightTexMaterial::Load (InStream& source)
{
	...
	PX2_END_DEBUG_STREAM_LOAD(LightTexMaterial, source);
}
//----------------------------------------------------------------------------
void LightTexMaterial::Link (InStream& source)
{
	Material::Link(source);
}
//----------------------------------------------------------------------------
void LightTexMaterial::PostLink ()
{
	Material::PostLink();
...
//----------------------------------------------------------------------------
bool LightTexMaterial::Register (OutStream& target) const
{
	return Material::Register(target);
}
//----------------------------------------------------------------------------
void LightTexMaterial::Save (OutStream& target) const
{
	...
	PX2_END_DEBUG_STREAM_SAVE(LightTexMaterial, target);
}
//----------------------------------------------------------------------------
int LightTexMaterial::GetStreamingSize (Stream &stream) const
{
...
// Profiles.
//----------------------------------------------------------------------------
int LightTexMaterial::msDx9VRegisters[6] = { 0, 4, 8, 9, 10, 11};
int LightTexMaterial::msOglVRegisters[6] = { 1, 5, 9, 10, 11, 12};
int LightTexMaterial::msOpenGLES2VRegisters[6] = { 0, 0, 0, 0, 0, 0};
int* LightTexMaterial::msVRegisters[Shader::MAX_PROFILES] =
{
	0,
	msDx9VRegisters,
	msDx9VRegisters,
...
std::string LightTexMaterial::msVPrograms[Shader::MAX_PROFILES] =
{
	// VS_NONE
	"",
	...
	// VS_2_0
	"vs_2_0\n"
	"def c12, 1.00000000, 0.50000000, 0.00000000, 0\n"
	"dcl_position0 v0\n"
	"dcl_texcoord0 v2\n"
	"dcl_normal0 v1\n"
	"mov r1.w, c12.x\n"
	...
	"mul r0.x, r0, r0.y\n"
	"min r0.x, r0, c12\n"
	"mul oD0.xyz, r0.z, c10\n"
	"dp4 oPos.w, r1, c3\n"
	"dp4 oPos.z, r1, c2\n"
	"dp4 oPos.y, r1, c1\n"
	"dp4 oPos.x, r1, c0\n"
	"max oT1.y, r0.x, c12.z\n"
	"mov oT0.xy, v2\n"
	"mov oD0.w, c12.x\n",
	...
	"	vertexColor.rgb = LightSpecular.rgb*pow(max(dotH, 0.0), LightSpecular.a);\n"
	"	vertexColor.a = 1.0;\n"
...
int LightTexMaterial::msAllPTextureUnits[2] = { 0, 1 };
int LightTexMaterial::msDx9PRegisters[1]= {0};
int LightTexMaterial::msOglPRegisters[1] = {1};
int LightTexMaterial::msOpenGLES2PRegisters[1] = {0};
int* LightTexMaterial::msPTextureUnits[Shader::MAX_PROFILES] =
{
	0,
	msAllPTextureUnits,
	msAllPTextureUnits,
...
int* LightTexMaterial::msPRegisters[Shader::MAX_PROFILES] =
{
	0,
	msDx9PRegisters,
	msDx9PRegisters,
...
std::string LightTexMaterial::msPPrograms[Shader::MAX_PROFILES] =
{
	// PS_NONE
	"",
	...
	"	mediump vec4 lastColor = color0*color1 + vertexColor;\n"
...
//----------------------------------------------------------------------------
std::string LightTexMaterial::msPPrograms1[Shader::MAX_PROFILES] =
{
	//msPPrograms[0],
	//msPPrograms[1],
	//msPPrograms[2],
...
//----------------------------------------------------------------------------
std::string LightTexMaterial::msPPrograsOGLESTest[2] =
{
	"varying mediump vec4 vertexColor;\n"
	"varying mediump vec2 vertexTCoord0;\n"
	"varying mediump vec2 vertexTCoord1;\n"
