...
typedef struct {
  INT32_JPG put_buffer;		/* current bit-accumulation buffer */
			 ...
			 c_derived_tbl ** pdtbl)
{
  ...
  while (huffsize[p]) {
     ...
     */
    if (((INT32_JPG) code) >= (((INT32_JPG) 1) << si))
      ERREXIT(cinfo, JERR_BAD_HUFF_TABLE);
    code <<= 1;
    si++;
  }
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  register INT32_JPG put_buffer = (INT32_JPG) code;
  ...
  put_buffer &= (((INT32_JPG) 1)<<size) - 1; /* mask off any extra bits in code */
