	...
	mNumCellsWidth(numCellsWidth)
{
	...
	for (int b = 0; b < mNumCellsWidth; b++)
	{
		for (int a = 0; a < mNumCellsLength; a++)
		{
			...
			APoint minPoint(left, but, 0.0f);
			APoint maxPoint(right, top, 0.0f);

			mCells.push_back(Cell(minPoint, maxPoint));
		}
	}
}
//-----------------------------------------------------------------------------
...
{
	int index = PositionToIndex(actor->LocalTransform.GetTranslate());
...
void CellSpace::UpdateActor(Actor *actor)
{
	...
	if (-1 != oldIndex)
	{
		int newIndex = PositionToIndex(actor->LocalTransform.GetTranslate());
