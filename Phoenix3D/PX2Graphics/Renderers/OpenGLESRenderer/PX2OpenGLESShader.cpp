								 ...
								 RendererData::SamplerState* currentSS)
{
	...
	for (int i = 0; i < numSamplers; ++i)
	{
		...
		bool hasMinMap = false;
		const Texture2D *tex2d = DynamicCast<Texture2D>(texture);
		if (tex2d)
		{
			hasMinMap = tex2d->HasMipmaps();
		}

		// Set the magfilter mode.
		Shader::SamplerFilter filter = shader->GetFilter(i);
		GLint glFilter = gOGLMinFilter[filter];

		if (!hasMinMap)
		{
			if (Shader::SF_NEAREST_NEAREST==filter ||
				Shader::SF_LINEAR_NEAREST==filter)
			{
				glFilter = GL_NEAREST;
			}

			if (Shader::SF_NEAREST_LINEAR==filter ||
				Shader::SF_LINEAR_LINEAR==filter)
			{
				glFilter = GL_LINEAR;
			}
		}

		if (glFilter != current.mMinFilter)
		{
			glTexParameteri(target, GL_TEXTURE_MIN_FILTER, glFilter);
			current.mMinFilter = glFilter;
		}
